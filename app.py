...
def plot_radar_rules(comparison: pd.DataFrame) -> go.Figure:
    ...
    for _, row in df.iterrows():
        ...
        # Convertir hex a rgba con transparencia para fillcolor
        h = color.lstrip("#")
...
def render_instance_block(data: Dict, show_decisions: bool, excel_label: str):
    ...
    st.markdown(
        ...
        La siguiente tabla resume todos los criterios de evaluación para cada regla de despacho.
        Los indicadores clave son &sum;U<sub>j</sub> (pedidos tardíos), &sum;T<sub>j</sub>
        (tardanza total), &sum;w<sub>j</sub>T<sub>j</sub> (tardanza ponderada) y el
        porcentaje de cumplimiento del SLA. <b>Menor valor es mejor</b> para todos excepto
        el cumplimiento del SLA, donde <b>mayor valor es mejor</b>.
        </p>
        """,
    ...
    for rule_tab, rule in zip(rule_tabs, all_schedules.keys()):
        with rule_tab:
            ...
            st.caption("Muestra la secuencia de operaciones programadas en cada recurso a lo largo del tiempo (segundos).")
            st.plotly_chart(
            ...
            st.subheader("Resultados por pedido")
            st.caption(
                "C\u2C7C = tiempo de finalización · F\u2C7C = tiempo de flujo · "
                "L\u2C7C = latencia · T\u2C7C = tardanza · U\u2C7C = incumplimiento SLA"
            )
            st.dataframe(all_results[rule], use_container_width=True)
            with st.expander("Ver operaciones programadas detalladas"):
...
st.markdown(
    ...
    <div style="background:#f0f4f8; border-left:4px solid #1a4f6e; padding:18px 24px;
                border-radius:6px; margin-top:4px; margin-bottom:12px;">
      <p style="margin:0 0 8px 0; font-size:15px; font-weight:700; color:#1a4f6e;">
        Criterio de desempeño principal: &sum;U<sub>j</sub> &mdash;
        número de pedidos que incumplen el SLA
      </p>
      <p style="margin:0; font-size:14px; color:#333; line-height:1.75;">
        El indicador central de análisis es la sumatoria de pedidos tardíos
        (&sum;U<sub>j</sub>), definida como la cantidad de pedidos cuyo tiempo de
        finalización supera la fecha de entrega comprometida (d<sub>j</sub>).
        Este criterio se justifica desde la perspectiva del servicio al cliente en
        restaurantes de comida rápida: el incumplimiento del tiempo de atención prometido
        —el SLA— afecta directamente la percepción del cliente, genera insatisfacción y
        deteriora indicadores operativos clave como el tiempo promedio de atención, la tasa
        de cumplimiento de órdenes y la retención de clientes en los distintos canales de
        servicio (AutoMac, Mostrador, Delivery y Pickup). Por esta razón, la comparación
        entre reglas de despacho se interpreta indicador por indicador, con énfasis en
        &sum;U<sub>j</sub>, &sum;T<sub>j</sub> (tardanza total acumulada),
        &sum;w<sub>j</sub>T<sub>j</sub> (tardanza ponderada por prioridad de pedido)
        y el porcentaje de cumplimiento del SLA.
      </p>
    </div>
    """,
    unsafe_allow_html=True,
