...
# real opera con select_candidate() (que sigue la lógica estándar de scheduling: SPT/WSPT
# por operación, CR con trabajo remanente, ATC en su versión clásica de Vepsalainen y
# Morton (1987) con pbar dinámico por instante de decisión y holgura truncada en cero)
...
def schedule_operations(
    ...
    def select_candidate(candidate_rows: List[Dict], current_time: float) -> List[Dict]:
        ...
        # pbar dinámico: promedio de las duraciones de las operaciones disponibles
        # en este instante de decisión, recalculado en cada llamada. Es la forma
        # clásica de Vepsalainen y Morton (1987): p̄(t) cambia a medida que se
        # programan operaciones y queda solo el subconjunto pendiente.
        ...
        for row in candidate_rows:
            ...
            # Holgura truncada en cero (max(., 0)): los pedidos ya atrasados reciben
            # factor exponencial = 1 y se desempatan únicamente por w_j / p_ij,
            # tal como en la formulación clásica de Vepsalainen y Morton (1987).
            row["Holgura_ATC"] = max(row["d_op"] - float(row["pij"]) - current_time, 0.0)
            # Tope numérico para que exp() no devuelva Inf con holguras enormes.
            ...
            if rule == "Holgura crítica por margen disponible + SPT":
                # Regla final sin umbral:
                # Grupo 1: pedidos con margen de cumplimiento (Holgura >= 0).
                #          Se prioriza menor holgura; desempate SPT.
                # Grupo 2: pedidos sin margen de cumplimiento (Holgura < 0).
                #          Se aplica SPT para reducir congestión operativa.
                if row["Holgura"] >= 0:
...
def render_instance_block(data: Dict, show_decisions: bool, excel_label: str):
    ...
    st.caption(
        ...
        "La regla de holgura crítica no usa umbral: separa pedidos con margen "
        "de cumplimiento frente al SLA y pedidos sin margen, aplicando SPT en este último grupo."
    )
...
def build_master_summary(all_instances_data: Dict[int, Dict]) -> pd.DataFrame:
    ...
    No usa un valor agregado; la lectura se hace por indicadores de servicio,
    especialmente ∑Uj, ∑Tj, ∑wjTj y Cumplimiento SLA (%).
    """
    rows = []
    ...
    return pd.DataFrame(rows).sort_values(
        ["Instancia", "∑Uj", "∑Tj", "Cmax", "Regla"]
    ).reset_index(drop=True)


def build_winners_table(master: pd.DataFrame) -> pd.DataFrame:
    """
    Para cada instancia, identifica la regla destacada por indicador clave.
    La lectura se centra en cumplimiento del SLA y servicio al cliente:
    ∑Uj indica cuántos pedidos incumplen, ∑Tj muestra la magnitud de la tardanza,
    ∑wjTj pondera esa tardanza por prioridad y el SLA (%) resume el cumplimiento.
    """
    rows = []
...
st.markdown(
    ...
    **Criterio de servicio:** en este proyecto, ∑Uj representa la cantidad de pedidos que
    incumplen el SLA. Por eso la comparación se interpreta indicador por indicador, como
    una revisión de cumplimiento y servicio al cliente: pedidos tardíos, tardanza total,
    tardanza ponderada y porcentaje de cumplimiento del SLA.
    """
)
...
try:
    ...
    # --------------------------------------------------------
        ...
        st.caption(
            "Una fila por combinación instancia × regla. La lectura se hace por indicadores: "
            "∑Uj mide pedidos que incumplen el SLA, ∑Tj mide la tardanza total, "
            "∑wjTj pondera la tardanza por prioridad y Cumplimiento SLA (%) resume el servicio al cliente."
        )
        st.dataframe(master, width="stretch")

        st.subheader("Reglas ganadoras por instancia")
        st.caption(
            "Para cada instancia, muestra qué regla obtiene el mejor valor en cada indicador clave. "
            "Si varias reglas empatan, se muestran juntas."
        )
